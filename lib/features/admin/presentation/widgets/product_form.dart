...
typedef ProductSaveCallback = void Function(
    Map<String, dynamic> data, List<XFile> imageFiles);
...
class _ProductFormState extends ConsumerState<ProductForm> {
  ...
  void initState() {
    ...
    _originalPriceController =
        TextEditingController(text: p?.originalPrice?.toString() ?? '');
    _stockController = TextEditingController(text: p?.stock.toString() ?? '');
    _descriptionController = TextEditingController(text: p?.description ?? '');
    _skuController = TextEditingController(
        text: p?.id ?? 'SKU-${DateTime.now().millisecondsSinceEpoch}');
  ...
  String _getImageUrl(String imagePath) {
    // Nếu đã là URL đầy đủ (từ Firebase) thì dùng ngay
    if (imagePath.startsWith('http')) {
      return imagePath;
    }
    // Nếu là đường dẫn tương đối từ server cũ
    if (imagePath.startsWith('/')) {
      return "${ApiConstants.baseUrl}$imagePath";
    }
    // Trường hợp mặc định (dù ít xảy ra)
  ...
  Future<void> _pickImages() async {
    ...
    if (pickedFiles.isNotEmpty) {
      if ((_existingImages.length + _imageFiles.length + pickedFiles.length) >
          5) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Chỉ được upload tối đa 5 ảnh.'),
      ...
      child: ListView(
        ...
        children: [
          ...
          TextFormField(
              ...
              decoration: const InputDecoration(labelText: 'Số lượng kho*'),
              keyboardType: TextInputType.number,
              validator: (v) => v!.isEmpty ? 'Bắt buộc' : null),
          const SizedBox(height: 16),
          TextFormField(
          ...
          categoriesAsync.when(
            data: (categories) {
              // ===== THÊM LOGIC ĐỒNG BỘ NÀY VÀO ĐÂY =====
              if (_newlyAddedCategoryId != null && categories.any((c) => c.id == _newlyAddedCategoryId)) {
                // Dùng addPostFrameCallback để gọi setState một cách an toàn sau khi build xong
                WidgetsBinding.instance.addPostFrameCallback((_) {
                  setState(() {
                    _selectedCategory = _newlyAddedCategoryId;
                    _newlyAddedCategoryId = null; // Reset biến tạm
                  });
                });
              }
              // ===============================================
              ...
              return DropdownButtonFormField<String>(
                ...
                items: [
                  ...
                  const DropdownMenuItem(
                    value: '__ADD_NEW__', // Một giá trị đặc biệt
                ...
                onChanged: (val) async {
                  if (val == '__ADD_NEW__') {
                    final newCategoryName = await _showAddNewDialog(context: context, title: 'Thêm Danh mục mới');
                    if (newCategoryName != null && mounted) {
                      try {
                        // Gọi API tạo mới
                        final newCategory = await ref.read(adminRepositoryProvider).createCategory(newCategoryName);
                        // Chỉ cần lưu ID mới và ra lệnh refresh, KHÔNG setState ở đây
          ...
          brandsAsync.when(
            data: (brands) {
              // ===== THÊM LOGIC ĐỒNG BỘ NÀY VÀO ĐÂY =====
              if (_newlyAddedBrandId != null && brands.any((b) => b.id == _newlyAddedBrandId)) {
                WidgetsBinding.instance.addPostFrameCallback((_) {
                  setState(() {
                    _selectedBrand = _newlyAddedBrandId;
                    _newlyAddedBrandId = null; // Reset biến tạm
                  });
                });
              }
              // ===============================================
              ...
              return DropdownButtonFormField<String>(
                ...
                items: [
                  ...
                  const DropdownMenuItem(
                    value: '__ADD_NEW__',
                    child: Text('➕ Thêm thương hiệu mới...',
                        style: TextStyle(
                            color: Colors.blue, fontWeight: FontWeight.bold)),
                  ),
                ],
                onChanged: (val) async {
                  if (val == '__ADD_NEW__') {
                    final newBrandName = await _showAddNewDialog(context: context, title: 'Thêm Thương hiệu mới');
                    if (newBrandName != null && mounted) {
                      try {
                        final newBrand = await ref.read(adminRepositoryProvider).createBrand(newBrandName);
                        // Chỉ cần lưu ID mới và ra lệnh refresh, KHÔNG setState ở đây
                        _newlyAddedBrandId = newBrand.id;
            ...
            GridView.builder(
              ...
              itemBuilder: (context, index) {
                return Stack(
                  ...
                  children: [
                    CachedNetworkImage(
                        imageUrl: _getImageUrl(_existingImages[index]),
                        fit: BoxFit.cover,
